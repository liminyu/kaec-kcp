...
module vars_e
 ...
 real(r8), allocatable :: coefte(:),coefti(:)
 real(r8), allocatable :: coefqfun(:),coefden(:),coefbeta(:),coefnb(:)
 real(r8), allocatable :: knot(:)
 real(r8), allocatable :: coefc(:)
 real(r8), allocatable :: coefaf0(:)
 real(r8), allocatable :: coefaf(:,:),coefbf(:,:)
 real(r8), allocatable :: coefgrr(:,:),coefgrt(:,:),coefgtt(:,:)
 real(r8), allocatable :: coefgrr0(:),coefgtt0(:)
 integer :: nx_sp,ksp,nc_sp,nf ! spline parameters for metric tensor
 integer :: nx_sp1,ksp1 ! spline parameters for profiles
end module vars_e
...
!
        function zf(t,x) !!Z/R0 of straight field line coordinates
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
           ...
           b7=cdbbsval(knot(1:nx1),ksp,coefbf(7,1:nx_sp),x,jderivs)
           b8=cdbbsval(knot(1:nx1),ksp,coefbf(8,1:nx_sp),x,jderivs)
          zf=b1*sin(t)+b2*sin(2.0*t)+b3*sin(3.0*t)&
...
!
        function pnor_i(x) ! normalized pressure, Pi/B^2
          implicit none
          real(r8) x, dum
          real(r8) :: pnor_i
          dum = inrcsval(gridx2,cscoefxpsi,x,0) ! psi
...
!
        ...
        subroutine profcoefdat
         ...
         parameter(k0=5)
!
          if (allocated(knotxr)) deallocate(knotxr)
          if (allocated(coefte)) deallocate(coefte)
          if (allocated(coefti)) deallocate(coefti)
          if (allocated(coefden)) deallocate(coefden)
          if (allocated(coefbeta)) deallocate(coefbeta)
          if (allocated(coefqfun)) deallocate(coefqfun)
          if (allocated(coefnb)) deallocate(coefnb)
         allocate(knotxr(nx0+k0),coefte(nx0),coefti(nx0))
         allocate(coefden(nx0),coefbeta(nx0))
         allocate(coefqfun(nx0),coefnb(nx0))
...
!
         ...
         read(550,*) ! Reading density and beta coefficients from the file
         do i=1,nx_sp1
...
!
          ...
          if (allocated(coefaf0)) deallocate(coefaf0)
          if (allocated(coefaf)) deallocate(coefaf)
          if (allocated(coefbf)) deallocate(coefbf)
          if (allocated(coefgrr0)) deallocate(coefgrr0)
          if (allocated(coefgrr)) deallocate(coefgrr)
          if (allocated(coefgrt)) deallocate(coefgrt)
          if (allocated(coefgtt0)) deallocate(coefgtt0)
          if (allocated(coefgtt)) deallocate(coefgtt)
         allocate(knot(nx0+k0),coefc(nx0))
         allocate(coefaf(nf0,nx0),coefaf0(nx0))
         allocate(coefbf(nf0,nx0))
         allocate(coefgrr0(nx0),coefgrr(nc0,nx0))
         allocate(coefgtt0(nx0),coefgtt(nc0,nx0))
         allocate(coefgrt(nc0,nx0))


         open(unit=53, file='fort.53', status='old')
         read(53,*)
         read(53,*)
         read(53,*) ksp,nx_sp,nc_sp,nf
         do i=1,nx_sp+ksp
            read(53,*) knot(i)
         enddo
         close(53)
...
!
        ...
        do i=1,nc_sp
          do j=1,nx_sp
             read(55,*) coefgrt(i,j),coefgrr(i,j),coefgtt(i,j)
          enddo
        enddo
         close(55)
