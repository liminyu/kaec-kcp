...
!
      ...
      if (ikct.eq.1) then
        ...
        aa(j,i)=aint
        call matcyy(jr,ir,xa,xb,nx,m1,m2,aint,aint_c) ! diamagnetic terms 20241216
        aa(j,i)=aa(j,i) - sqrt(lam_omg)*aint ! modified by diamagnetic terms 20241216  
      endif
...
!
       ...
       aint_m=aint1+aint2+2.0*1.0*aint3/e**2+aint5
...
!      eps=0.20D0
!      eps=0.375
       pr=1.0D0*sqrt(pi0)*rhoi_bar/e/e
...
!        write(30,*)'x0=',x0,'x1=',x1,'x2=',x2
!
        f0=pr*(ppnor_i(x0))*(lammbda8(x0,m1,m2,j1)*femp(j,m1,x0)*femp(i,m2,x0))
        f1=pr*(ppnor_i(x1))*(lammbda8(x1,m1,m2,j1)*femp(j,m1,x1)*femp(i,m2,x1))
        f2=pr*(ppnor_i(x2))*(lammbda8(x2,m1,m2,j1)*femp(j,m1,x2)*femp(i,m2,x2))
!
        aint_c=aint_c+h/3.0D0*(f0+4.0D0*f1+f2)
      enddo           
