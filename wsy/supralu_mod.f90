    ...
    subroutine sparseC16_eigen_k(Amat, B0mat, x, lambda,matsize,rank, base,tol, nmax, ier)
     ...
     do k = 1, nmax
         ...
         !if (k>1) then
          ...
          Bmat % values = B0mat % values
         !endif 
         Cmat % values = Amat % values - lambda * B0mat % values + ee*Dmat % values
         call sparseC16_A_dot_x(Cmat, x, rhs, ier)
         residue = sqrt( dot_product(rhs, rhs) )
         if(residue < tol) exit
     ...
     !
      ...
      write(1005,*) nog,i2
      write(1005,*) lambda
      write(1005,*) ii3,real(rho),real(rho),imag(rho)
      close(1005)
!
      nmax = k
      tol = residue
...
!      complex(r8) aa(0:1000)
      ...
      ikct = 1 ! 0: simple iteration for fixed lambda in Amat 
...
!c      deallocate(rhs, x, res)
      ...
      deallocate(C8tt) ! diamagnetic_tt
      deallocate(C8rr) ! diamagnetic_rr
      deallocate(C8tr) ! diamagnetic_tr
      if (ikct.eq.1) then
        deallocate(dd)
      endif
...
!
      call xrange(jr,ir,xa,xb,nx)
!  
       call matcyy(jr,ir,xa,xb,nx,m1,m2,aint,aint_c) ! diamagnetic terms 20241216
       write(28,*) real(aint_c)
       write(30,*) j,i,real(aint)
!       
    enddo
   enddo
!
...
!
      ...
      call matcyy(jr,ir,xa,xb,nx,m1,m2,aint,aint_c)
      write(300,*) j,i,real(aint)
      if (ikct.eq.1) then
        call mataayy(jr,ir,xa,xb,nx,m1,m2,aint,aint_c,aint_m,aint_k,aint_kp)
        aa(j,i)=aint_m
        !call matcyy(jr,ir,xa,xb,nx,m1,m2,aint,aint_c) ! diamagnetic terms 20241216
        !aa(j,i)=aa(j,i)- sqrt(lam)*aint ! modified by diamagnetic terms 20241216
        dd(j,i)=aint_k
      else
        call matayy(jr,ir,xa,xb,nx,m1,m2,aint,aint_c,aint_m,aint_k)
        aa(j,i)=aint
...
!      write(260,*) j,i,real(aint_m)-real(sqrt(lam)*aint) ! modified by diamagnetic terms 20241216
!      write(261,*) j,i,real(aint_k),aimag(aint_k)-aimag(sqrt(lam)*aint)
       write(260,*) j,i,real(aint_m)
       dum = 0.0D0
       write(261,*) j,i,dum,aimag(aint_k)
...
!
       ...
       !aint=aint_m + aint_k
       aint=aint_m+cmplx(0.0,aimag(aint_k))
...
!
    subroutine matcyy(j,i,xa,xb,nx,m1,m2,aint,aint_c)
      implicit none
      integer i,j,nx,m1,m2,nxo2,jj,j1
      real(r8) :: xa,xb,h,am1,am2,g,eps,eps2,pr
      real(r8) :: x0,x1,x2
!      real(r8) :: fem,femp,fempp,den,gamma1,delta1,theta1,lammbda1
      !complex(r8) :: aint,f0,f1,f2,rho
      complex(r8) :: aint,f0,f1,f2
...
!      eps=0.20D0
!      eps=0.375
       pr=4.0D0*sqrt(pi0)*rhoi_bar/e/e
...
!        write(30,*)'x0=',x0,'x1=',x1,'x2=',x2
!  
        f0=pr*den(x0)*(tfunp(x0)+tfun(x0)*logdenp(x0))*&
          (fem(j,m1,x0)*fem(i,m2,x0)*gamma8(x0,m1,m2,j1)- &
          fem(j,m1,x0)*femp(i,m2,x0)*delta8(x0,m1,m2,j1)+ &
          theta8(x0,m1,m2,j1)*femp(j,m1,x0)*fem(i,m2,x0)+ &
          lammbda8(x0,m1,m2,j1)*femp(j,m1,x0)*femp(i,m2,x0))
        f1=pr*den(x1)*(tfunp(x1)+tfun(x1)*logdenp(x1))*&
          (fem(j,m1,x1)*fem(i,m2,x1)*gamma8(x1,m1,m2,j1)- &
          fem(j,m1,x1)*femp(i,m2,x1)*delta8(x1,m1,m2,j1)+ &
          theta8(x1,m1,m2,j1)*femp(j,m1,x1)*fem(i,m2,x1)+ &
          lammbda8(x1,m1,m2,j1)*femp(j,m1,x1)*femp(i,m2,x1))
        f2=pr*den(x2)*(tfunp(x2)+tfun(x2)*logdenp(x2))*&
          (fem(j,m1,x2)*fem(i,m2,x2)*gamma8(x2,m1,m2,j1)- &
          fem(j,m1,x2)*femp(i,m2,x2)*delta8(x2,m1,m2,j1)+ &
          theta8(x2,m1,m2,j1)*femp(j,m1,x2)*fem(i,m2,x2)+ &
          lammbda8(x2,m1,m2,j1)*femp(j,m1,x2)*femp(i,m2,x2))
!  
        aint=aint+h/3.0D0*(f0+4.0D0*f1+f2)
      enddo  
!
...
!        write(30,*)'x0=',x0,'x1=',x1,'x2=',x2
!
        f0=pr*den(x0)*(tfunp(x0)+tfun(x0)*logdenp(x0))*(lammbda8(x0,m1,m2,j1)*femp(j,m1,x0)*femp(i,m2,x0))
        f1=pr*den(x1)*(tfunp(x1)+tfun(x1)*logdenp(x1))*(lammbda8(x1,m1,m2,j1)*femp(j,m1,x1)*femp(i,m2,x1))
        f2=pr*den(x2)*(tfunp(x2)+tfun(x2)*logdenp(x2))*(lammbda8(x2,m1,m2,j1)*femp(j,m1,x2)*femp(i,m2,x2))
!
        aint_c=aint_c+h/3.0D0*(f0+4.0D0*f1+f2)
      enddo           

!        return
        end subroutine matcyy
!
