          module test2_mod
           ...
           contains
             ...
             use orbit_eq_mod
             implicit none
             integer, parameter :: nstep2=51,nstep3=201, nseca=4
             ...
             complex(8) :: sdum3,sdum4,sdum5,fdum,fdum1
             real(8) :: runt2
             real(8) :: xa,xb,xw
             real(8) :: ra,rb
             real(8) :: eps
...
!            mode numbers
            ...
            if (allocated(pmode)) then
             ...
             allocate(aint_KCT4D(0:nog,0:nog,nomode,nomode))
               if (allocated(temp_aint_KCT4D)) then
                                   ...
                                   if(il.eq.0)then ! fixed 1->0 20230304 yulm
                                    ...
                                    if(ra <= xlb .and. rb >= xla) then ! nes FOW cond                                     
                                       ...
                                       dum4 = dLa(iname)
                                        ...
                                        fdum1= (dume)**3*dE0(iname)/ekev*dpz0(iname)/&
                                                  e/e*dum4*tau_theta1(iname)*dum2
                                      ...
                                      do ip = 1, nopmode ! ip-> p for matrix element M'_{i,l}^{k,m}
                                        ...
                                        if(ra <= xlb .and. rb >= xla) then ! nes FOW cond
                                         ...
                                         !$OMP END PARALLEL DO 
                                           ...
                                           ylmp(ip,iarray,im,il)/&
                                           (-omg1+float(pmode(ip))*w_theta1(iname)+&
                                            float(ntor)*w_phi1(iname))*fdum1/2.0/omg
                                   ...
                                   do il1 = 0, nog  ! il1->i
                                      do ip = 1, nopmode ! ip-> p for
                                         ...
                                         if ((sdum3 /= cmplx(0.0,0.0)) .and. &
                                            ...
                                            1.0/1.0*sdum4*fylmpp(ip,iarray,im,il)
...
!
              ...
              if (proc_num == 0) then 
                 ...
                 write(8,*) 'Cn1:', Cn1
                 write(1001,78)
                 write(1001,*) nprt1,ik, vA_bar, Cn1
                close(1001)
               endif
             enddo
           else
