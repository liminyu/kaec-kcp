...
!
      function pfpe(Ek,Lamb,psi,x)
       implicit none
       real,parameter ::pi=3.14159265358979,two_pi=6.28318530717959d0
       !common/coef_dist/Cn
       !real(r8) Ek, Lamb,psi,Cn
       real(r8) Ek, Lamb,psi,x
       real(r8) :: pfpe
!      psi = pol/pw
!      Ek = E/ekev
       !pfpe = - 8.0/sqrt(pi)/Cn*&
       ...
       !pfpe = -1.0/pi**1.5/t_h_b(x)**2*den_h_b(x)*&
       !         expE(Ek)*exp(1.0/t_h_b(x)) ! maxwell, isotropic, d3d
      endfunction pfpe
!
!
      function pfpz(Ek,Lamb,psi,x) ! partial F / partial Pz
       implicit none
       real,parameter ::pi=3.14159265358979,two_pi=6.28318530717959d0
       !common/coef_dist/Cn
       !real(r8) Ek, Lamb,psi,Cn
...
!      Ek = E/ekev
       ...
       !pfpz = 1.0/pi**1.5/t_h_b(x)*qfun(x)/jr2(x)&
       !      *expE(Ek)*exp(1.0/t_h_b(x))&
       !      *(denp_h_b(x)&
       !       -1.0/t_h_b(x)&
       !       *tp_h_b(x)*den_h_b(x)&
       !       +1.0/t_h_b(x)**2&
       !       *tp_h_b(x)*den_h_b(x)*Ek)! maxwell,isotropic d3d
      endfunction pfpz
!
       function pfpz1(Ek,Lamb,psi,x)
        implicit none
        !common/aspect/e!aspect rate:e=a/R_0
        !real(r8) Ek, psi,e,Lamb
        real(r8) Ek, psi,Lamb,x
        real(r8) :: pfpz1
!       psi = pol/pw
!      Ek = E/ekev
