module vars
  ...
  real(sp) :: rhoh_bar,vh,rmaj,omeg0,ekev,zprt,zprt0,bkg,prot,vA_bar,ee
  ...
  real(sp), allocatable :: tdum(:),Rdum(:),Zdum(:),&
   ...
   real(sp), allocatable :: tdum1(:), Rdum1(:),Zdum1(:),&
                             ...
                             psidum1(:),thetadum1(:),&
                             psirdum1(:)
   real(sp), allocatable :: psirdum2(:,:),thetadum2(:,:),phidum2(:,:)
...
module vars_e
 ...
 complex(r8) :: omg,lam_omg !omega_mode, resisitive eta, eigen_value
 complex(kind=4) :: rho
 real(r8) :: q0,q1,cq1,xini,xend !qfundat
 real(r8) :: delta ! step of raidal grids xgrid1
 real(r8) :: dtheta ! step of theta grids for gyro-average
 real(r8) :: b0_eff,b0_h ! beta_eff=P/B^2 at core 
 ...
 real(r8), allocatable :: knotxr(:)
 real(r8), allocatable :: coefcste(:,:),coefcsti(:,:)
 real(r8), allocatable :: coefcsqfun(:,:),coefcsden(:,:),coefcsbeta(:,:),coefcsnb(:,:)
 real(r8), allocatable :: coefcsrot(:,:),coefcstb(:,:)
 real(r8), allocatable :: knot(:)
 real(r8), allocatable :: xa_cs(:),xa_cs0(:)
 real(r8), allocatable :: coefcsc(:,:) ! coefcsc:coefficients for spline of Jr2(x)
 real(r8), allocatable :: coefcsaf0(:,:)
 real(r8), allocatable :: coefcsbf0(:,:)
 real(r8), allocatable :: coefcsaf(:,:,:),coefcsbf(:,:,:)
 real(r8), allocatable :: coefcsgrr(:,:,:),coefcsgrt(:,:,:),coefcsgtt(:,:,:)
 real(r8), allocatable :: coefcsgrr0(:,:),coefcsgtt0(:,:)
 integer :: nx_cs,kcs,nc_cs,nf ! spline parameters for metric tensor
 integer :: nx_cs0,nx_sp1,ksp1 ! spline parameters for profiles
end module vars_e
...
!
        ...
        function rf(t,x) !!R/R0 of straight field line coordinates
          ...
          integer,parameter :: jderivs=0
          integer :: nx1
          real(r8) t,x,dx
          real(r8) :: rf
          real(r8) :: a0,a1,a2,a3,a4,a5,a6,a7,a8
          real(r8) :: b1,b2,b3,b4,b5,b6,b7,b8
!
          if(x.le.1.0D-3) x=1.0D-3
          if(x.ge.1.0D0) x=1.0-1e-06
          a0=inrcsval(xa_cs,coefcsaf0(1:4,1:nx_cs-1),x,jderivs)
          a1=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,1),x,jderivs)
          a2=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,2),x,jderivs)
          a3=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,3),x,jderivs)
          a4=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,4),x,jderivs)
          !a5=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,5),x,jderivs)
          !a6=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,6),x,jderivs)
          !a7=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,7),x,jderivs)
          !a8=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,8),x,jderivs)
         !   a9=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs,9),x,jderivs)
         !  a10=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs,10),x,jderivs)
         !  a2=cdbbsval(knot(1:nx1),ksp,coefaf(2,1:nx_sp),x,jderivs)
         !  a3=cdbbsval(knot(1:nx1),ksp,coefaf(3,1:nx_sp),x,jderivs)
         !   a4=cdbbsval(knot(1:nx1),ksp,coefaf(4,1:nx_sp),x,jderivs)
         !   a5=cdbbsval(knot(1:nx1),ksp,coefaf(5,1:nx_sp),x,jderivs)
         !   a6=cdbbsval(knot(1:nx1),ksp,coefaf(6,1:nx_sp),x,jderivs)
         !   a7=cdbbsval(knot(1:nx1),ksp,coefaf(7,1:nx_sp),x,jderivs)
         !   a8=cdbbsval(knot(1:nx1),ksp,coefaf(8,1:nx_sp),x,jderivs)
           !b1=cdbbsval(knot(1:nx1),ksp,coefbf(1,1:nx_sp),x,jderivs)
           !b2=cdbbsval(knot(1:nx1),ksp,coefbf(2,1:nx_sp),x,jderivs)
           !b3=cdbbsval(knot(1:nx1),ksp,coefbf(3,1:nx_sp),x,jderivs)
           !b4=cdbbsval(knot(1:nx1),ksp,coefbf(4,1:nx_sp),x,jderivs)
           !b5=cdbbsval(knot(1:nx1),ksp,coefbf(5,1:nx_sp),x,jderivs)
           !b6=cdbbsval(knot(1:nx1),ksp,coefbf(6,1:nx_sp),x,jderivs)
           !b7=cdbbsval(knot(1:nx1),ksp,coefbf(7,1:nx_sp),x,jderivs)
           !b8=cdbbsval(knot(1:nx1),ksp,coefbf(8,1:nx_sp),x,jderivs)
           !rf=a0+a1*cos(t)+a2*cos(2.0*t)+a3*cos(3.0*t)&
           !   +a4*cos(4.0*t)+a5*cos(5.0*t)+a6*cos(6.0*t)&
           !   +a7*cos(7.0*t)+a8*cos(8.0*t)
           rf = a0+a1*cos(t)+a2*cos(2.0*t) +&
               a3*cos(3.0*t)+a4*cos(4.0*t)
          !dx=delta/float(nq)
!         x=0.5D0
          !n=x/dx+0.01D0
!
...
!
        function zf(t,x) !!Z/R0 of straight field line coordinates
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !   nx1=nx_sp+ksp
           !a0=cdbbsval(knot(1:nx1),ksp,coefaf0(1:nx_sp),x,jderivs)
           !a1=cdbbsval(knot(1:nx1),ksp,coefaf(1,1:nx_sp),x,jderivs)
           !a2=cdbbsval(knot(1:nx1),ksp,coefaf(2,1:nx_sp),x,jderivs)
           !a3=cdbbsval(knot(1:nx1),ksp,coefaf(3,1:nx_sp),x,jderivs)
           !a4=cdbbsval(knot(1:nx1),ksp,coefaf(4,1:nx_sp),x,jderivs)
           !a5=cdbbsval(knot(1:nx1),ksp,coefaf(5,1:nx_sp),x,jderivs)
         ...
         !   b7=cdbbsval(knot(1:nx1),ksp,coefbf(7,1:nx_sp),x,jderivs)
         !   b8=cdbbsval(knot(1:nx1),ksp,coefbf(8,1:nx_sp),x,jderivs)
         b0=inrcsval(xa_cs,coefcsbf0(1:4,1:nx_cs-1),x,jderivs)
         b1=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,1),x,jderivs)
         b2=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,2),x,jderivs)
         b3=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,3),x,jderivs)
         b4=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,4),x,jderivs)
         ! b5=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,5),x,jderivs)
         ! b6=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,6),x,jderivs)
         ! b7=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,7),x,jderivs)
         ! b8=inrcsval(xa_cs,coefcsbf(1:4,1:nx_cs-1,8),x,jderivs)
         !   zf=b0+b1*sin(t)+b2*sin(2.0*t)+b3*sin(3.0*t)&
         !      +b4*sin(4.0*t)+b5*sin(5.0*t)+b6*sin(6.0*t)&
         !      +b7*sin(7.0*t)+b8*sin(8.0*t)
         zf = b0+b1*sin(t)+b2*sin(2.0*t)+&
             b3*sin(3.0*t)+b4*sin(4.0*t)
          zf=e*zf
!         
...
!
         ...
         !   a8=cdbbsval(knot(1:nx1),ksp,coefaf(8,1:nx_sp),x,jderivs)
          ...
          if (e==0.0D0) then
            ...
            !   rfpr=(a0+a1*cos(t)+a2*cos(2.0*t)+a3*cos(3.0*t)&
            !   +a4*cos(4.0*t)+a5*cos(5.0*t)+a6*cos(6.0*t)&
            !   +a7*cos(7.0*t)+a8*cos(8.0*t))/e      
            rfpr=(a0+a1*cos(t)+a2*cos(2.0*t)+&
                  +a3*cos(3.0*t)+a4*cos(4.0*t))/e
          endif
          !if (e==0.0D0) then
          !rfpr=cos(t)
          !else
...
!
         ...
         !    a8=cdbbsval(knot(1:nx1),ksp,coefaf(8,1:nx_sp),x,jderivs)
            ...
            ! a7=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,7),x,jderivs)
            ! a8=inrcsval(xa_cs,coefcsaf(1:4,1:nx_cs-1,8),x,jderivs)
         !   rfpt=-(a1*sin(t)+2.0*a2*sin(2.0*t)+3.0*a3*sin(3.0*t)&
         !      +4.0*a4*sin(4.0*t)+5.0*a5*sin(5.0*t)+6.0*a6*sin(6.0*t)&
         !      +7.0*a7*sin(7.0*t)+8.0*a8*sin(8.0*t))
!
!          rfpt=-x*sin(t)-2.0*x*eta_r(x)*sin(2.0*t)
          ...
          rfpt=-a1*sin(t)-2.0*a2*sin(2.0*t)-&
                3.0*a3*sin(3.0*t)-4.0*a4*sin(4.0*t)
        endfunction rfpt
!
!
...
!
        function jfpx(t,x)
          ...
          real(r8) x,t
          real(r8) ::jfpx
          jfpx=2.0*rfpx(t,x)*jr2(x)*rf(t,x)+&
                jr2p(x)*rf(t,x)**2
...
!
        function jfpt(t,x)!jacobian
          ...
          real(r8) x,t
          real(r8) ::jfpt
          jfpt=2.0*jr2(x)*rf(t,x)*rfpt(t,x)
...
!
        function grr(t,x)!metrix coefficients
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !   nx1=nx_sp+ksp
         !   a0=cdbbsval(knot(1:nx1),ksp,coefgrr0(1:nx_sp),x,jderivs)
         !   a1=cdbbsval(knot(1:nx1),ksp,coefgrr(1,1:nx_sp),x,jderivs)
         !   a2=cdbbsval(knot(1:nx1),ksp,coefgrr(2,1:nx_sp),x,jderivs)
         !   a3=cdbbsval(knot(1:nx1),ksp,coefgrr(3,1:nx_sp),x,jderivs)
         !   a4=cdbbsval(knot(1:nx1),ksp,coefgrr(4,1:nx_sp),x,jderivs)
         !   a5=cdbbsval(knot(1:nx1),ksp,coefgrr(5,1:nx_sp),x,jderivs)
         !   a6=cdbbsval(knot(1:nx1),ksp,coefgrr(6,1:nx_sp),x,jderivs)
         !   a7=cdbbsval(knot(1:nx1),ksp,coefgrr(7,1:nx_sp),x,jderivs)
         !   a8=cdbbsval(knot(1:nx1),ksp,coefgrr(8,1:nx_sp),x,jderivs)
         a0=inrcsval(xa_cs,coefcsgrr0(1:4,1:nx_cs-1),x,jderivs)
         a1=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,1),x,jderivs)
         a2=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,2),x,jderivs)
         a3=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,3),x,jderivs)
         a4=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,4),x,jderivs)
         ! a5=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,5),x,jderivs)
         ! a6=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,6),x,jderivs)
         ! a7=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,7),x,jderivs)
         ! a8=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,8),x,jderivs)
         !   grr=a0+a1*cos(t)+a2*cos(2.0*t)+a3*cos(3.0*t)&
         !      +a4*cos(4.0*t)+a5*cos(5.0*t)+a6*cos(6.0*t)&
         !      +a7*cos(7.0*t)+a8*cos(8.0*t)
          !grr=1.0
          !grr=1.0+2.0*ddel(x)*cos(t)
          grr=a0+a1*cos(t)+a2*cos(2.0*t)+&
              a3*cos(3.0*t)+a4*cos(4.0*t)
        endfunction grr
!
        function grt(t,x)
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !   nx1=nx_sp+ksp
         !   a1=cdbbsval(knot(1:nx1),ksp,coefgrt(1,1:nx_sp),x,jderivs)
         !   a2=cdbbsval(knot(1:nx1),ksp,coefgrt(2,1:nx_sp),x,jderivs)
         !   a3=cdbbsval(knot(1:nx1),ksp,coefgrt(3,1:nx_sp),x,jderivs)
         !   a4=cdbbsval(knot(1:nx1),ksp,coefgrt(4,1:nx_sp),x,jderivs)
         !   a5=cdbbsval(knot(1:nx1),ksp,coefgrt(5,1:nx_sp),x,jderivs)
         !   a6=cdbbsval(knot(1:nx1),ksp,coefgrt(6,1:nx_sp),x,jderivs)
         !   a7=cdbbsval(knot(1:nx1),ksp,coefgrt(7,1:nx_sp),x,jderivs)
         !   a8=cdbbsval(knot(1:nx1),ksp,coefgrt(8,1:nx_sp),x,jderivs)
         a1=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,1),x,jderivs)
         a2=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,2),x,jderivs)
         a3=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,3),x,jderivs)
         a4=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,4),x,jderivs)
         ! a5=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,5),x,jderivs)
         ! a6=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,6),x,jderivs)
         ! a7=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,7),x,jderivs)
         ! a8=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,8),x,jderivs)
         !   grt=(a1*sin(t)+a2*sin(2.0*t)+a3*sin(3.0*t)&
         !      +a4*sin(4.0*t)+a5*sin(5.0*t)+a6*sin(6.0*t)&
         !      +a7*sin(7.0*t)+a8*sin(8.0*t))/x
          grt=(a1*sin(t)+a2*sin(2.0*t)+& 
               a3*sin(3.0*t)+a4*sin(4.0*t))/x
...
!
        function gtt(t,x)
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !   nx1=nx_sp+ksp
         !   a0=cdbbsval(knot(1:nx1),ksp,coefgtt0(1:nx_sp),x,jderivs)
         !   a1=cdbbsval(knot(1:nx1),ksp,coefgtt(1,1:nx_sp),x,jderivs)
         !   a2=cdbbsval(knot(1:nx1),ksp,coefgtt(2,1:nx_sp),x,jderivs)
         !   a3=cdbbsval(knot(1:nx1),ksp,coefgtt(3,1:nx_sp),x,jderivs)
         !   a4=cdbbsval(knot(1:nx1),ksp,coefgtt(4,1:nx_sp),x,jderivs)
         !   a5=cdbbsval(knot(1:nx1),ksp,coefgtt(5,1:nx_sp),x,jderivs)
         !   a6=cdbbsval(knot(1:nx1),ksp,coefgtt(6,1:nx_sp),x,jderivs)
         !   a7=cdbbsval(knot(1:nx1),ksp,coefgtt(7,1:nx_sp),x,jderivs)
         !   a8=cdbbsval(knot(1:nx1),ksp,coefgtt(8,1:nx_sp),x,jderivs)
         a0=inrcsval(xa_cs,coefcsgtt0(1:4,1:nx_cs-1),x,jderivs)
         a1=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,1),x,jderivs)
         a2=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,2),x,jderivs)
         a3=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,3),x,jderivs)
         a4=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,4),x,jderivs)
         ! a5=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,5),x,jderivs)
         ! a6=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,6),x,jderivs)
         ! a7=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,7),x,jderivs)
         ! a8=inrcsval(xa_cs,coefcsgtt(1:4,1:nx_cs-1,8),x,jderivs)
         !   gtt=(a0+a1*cos(t)+a2*cos(2.0*t)+a3*cos(3.0*t)&
         !      +a4*cos(4.0*t)+a5*cos(5.0*t)+a6*cos(6.0*t)&
         !      +a7*cos(7.0*t)+a8*cos(8.0*t))/x**2
          gtt=(a0+a1*cos(t)+a2*cos(2.0*t)+a3*cos(3.0*t)&
               +a4*cos(4.0*t))/x**2
...
!
          do j=1,na
             write(40,*) rf(t(j),x(n)),zf(t(j),x(n))
          enddo
...
!
        function jr2(x)
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !   nx1=nx_sp+ksp
         !   jr2=cdbbsval(knot(1:nx1),ksp,coefc(1:nx_sp),x,jderivs)
          jr2=inrcsval(xa_cs,coefcsc(1:4,1:nx_cs-1),x,jderivs)
          !jr2=x
        endfunction jr2
!
        function jr20(t,x)
...
!
        function jr2p(x)
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !   nx1=nx_sp+ksp
         !   jr2p=cdbbsval(knot(1:nx1),ksp,coefc(1:nx_sp),x,jderivs)
           jr2p=inrcsval(xa_cs,coefcsc(1:4,1:nx_cs-1),x,jderivs)
          !jr2p=1.0
        endfunction jr2p
!
        ...
        function grrpt(t,x)
         ...
         !    a8=cdbbsval(knot(1:nx1),ksp,coefgrr(8,1:nx_sp),x,jderivs)
          a1=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,1),x,jderivs)
            ...
            ! a7=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,7),x,jderivs)
            ! a8=inrcsval(xa_cs,coefcsgrr(1:4,1:nx_cs-1,8),x,jderivs)
            ! grrpt=-(a1*sin(t)+2.0*a2*sin(2.0*t)+3.0*a3*sin(3.0*t)&
            !   +4.0*a4*sin(4.0*t)+5.0*a5*sin(5.0*t)+6.0*a6*sin(6.0*t)&
            !   +7.0*a7*sin(7.0*t)+8.0*a8*sin(8.0*t)) 
          grrpt=-(a1*sin(t)+2.0*a2*sin(2.0*t)+&
               3.0*a3*sin(3.0*t)+4.0*a4*sin(4.0*t))
          !grrpt=0.0
...
!
        function gtrpptx(t,x)
          implicit none
          integer,parameter :: jderivs=0, jderivs1=1
          integer :: nx1
          real(r8) t,x
          real(r8) ::gtrpptx
          real(r8) :: a0,a1,a2,a3,a4,a5,a6,a7,a8
...
!
          if(x.le.1.0D-3) x=1.0D-3
           if(x.ge.1.0D0) x=1.0-1e-06
            ...
            ! a7=cdbbsval(knot(1:nx1),ksp,coefgrt(7,1:nx_sp),x,jderivs)
            ! a8=cdbbsval(knot(1:nx1),ksp,coefgrt(8,1:nx_sp),x,jderivs)
            ! b1=cdbbsval(knot(1:nx1),ksp,coefgrt(1,1:nx_sp),x,jderivs1)
            ! b2=cdbbsval(knot(1:nx1),ksp,coefgrt(2,1:nx_sp),x,jderivs1)
            ! b3=cdbbsval(knot(1:nx1),ksp,coefgrt(3,1:nx_sp),x,jderivs1)
            ! b4=cdbbsval(knot(1:nx1),ksp,coefgrt(4,1:nx_sp),x,jderivs1)
            ! b5=cdbbsval(knot(1:nx1),ksp,coefgrt(5,1:nx_sp),x,jderivs1)
            ! b6=cdbbsval(knot(1:nx1),ksp,coefgrt(6,1:nx_sp),x,jderivs1)
            ! b7=cdbbsval(knot(1:nx1),ksp,coefgrt(7,1:nx_sp),x,jderivs1)
            ! b8=cdbbsval(knot(1:nx1),ksp,coefgrt(8,1:nx_sp),x,jderivs1)
            a1=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,1),x,jderivs)
            a2=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,2),x,jderivs)
            a3=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,3),x,jderivs)
            a4=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,4),x,jderivs)
            ...
            ! a7=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,7),x,jderivs)
            ! a8=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,8),x,jderivs)
            b1=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,1),x,jderivs1)
            b2=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,2),x,jderivs1)
            b3=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,3),x,jderivs1)
            b4=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,4),x,jderivs1)
            ! b5=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,5),x,jderivs1)
            ! b6=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,6),x,jderivs1)
            ! b7=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,7),x,jderivs1)
            ! b8=inrcsval(xa_cs,coefcsgrt(1:4,1:nx_cs-1,8),x,jderivs1)
         !   f1=-(a1*cos(t)+2.0*a2*cos(2.0*t)+3.0*a3*cos(3.0*t)&
         !      +4.0*a4*cos(4.0*t)+5.0*a5*cos(5.0*t)+6.0*a6*cos(6.0*t)&
         !      +7.0*a7*cos(7.0*t)+8.0*a8*cos(8.0*t))/x**2
         !   f2=(b1*cos(t)+2.0*b2*cos(2.0*t)+3.0*b3*cos(3.0*t)&
         !      +4.0*b4*cos(4.0*t)+5.0*b5*cos(5.0*t)+6.0*b6*cos(6.0*t)&
         !      +7.0*b7*cos(7.0*t)+8.0*b8*cos(8.0*t))/x
          f1=-(a1*cos(t)+2.0*a2*cos(2.0*t)+&
               3.0*a3*cos(3.0*t)+4.0*a4*cos(4.0*t))/x**2
            f2=(b1*cos(t)+2.0*b2*cos(2.0*t)+&
               3.0*b3*cos(3.0*t)+4.0*b4*cos(4.0*t))/x
         !f2=(b1*cos(t)+2.0*b2*cos(2.0*t))/x
          gtrpptx=f1+f2
...
!
          if (jp==0) then
             gamma8=float(k*m**2)*C8tt(jp,n)
             return
          elseif (k-m+jp==0)then
             gamma8=float(k*m**2)*C8tt(jp,n)/2.0 &
                    -1.0/x*float(k*m)*C8tr(jp,n)/2.0
          elseif (k-m-jp==0)then
             gamma8=float(k*m**2)*C8tt(jp,n)/2.0 &
                    +1.0/x*float(k*m)*C8tr(jp,n)/2.0
          endif
...
!
          if (jp==0) then
             delta8=x*float(k)*C8tt(jp,n)
             return
          elseif (k-m+jp==0)then
             delta8=-float(k*m)*C8tr(jp,n)/2.0 &
                    +x*float(k)*C8tt(jp,n)/2.0
          elseif(k-m-jp==0)then
             delta8=float(k*m)*C8tr(jp,n)/2.0 &
                    +x*float(k)*C8tt(jp,n)/2.0
          endif
...
!
          if (jp==0) then
             theta8=-float(m)*C8rr(jp,n)/x
             return
          elseif (k-m+jp==0)then
             theta8=-float(m**2)*C8tr(jp,n)/2.0 &
                    -float(m)/x*C8rr(jp,n)/2.0
          elseif(k-m-jp==0)then
             theta8=float(m**2)*C8tr(jp,n)/2.0 &
                   -float(m)/x*C8rr(jp,n)/2.0
          endif
...
!
          if (jp==0) then
             lammbda8=float(m)*C8rr(jp,n)
             return
          elseif (k-m+jp==0)then
             lammbda8=float(m)*C8rr(jp,n)/2.0 &
                      +x*C8tr(jp,n)/2.0
          elseif (k-m-jp==0)then
             lammbda8=-float(m)*C8rr(jp,n)/2.0 &
                      -x*C8tr(jp,n)/2.0
...
!
        function beta(x) ! normalized pressure, P/B^2
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !   nx1=nx_sp1+ksp1
         !   beta=cdbbsval(knotxr(1:nx1),ksp1,coefbeta(1:nx_sp1),x,jderivs)
          beta = inrcsval(xa_cs0,coefcsbeta(1:4,1:nx_cs0-1),x,jderivs)
          beta=b0_eff*beta
...
!
        function betap(x)
          ...
          if(x.ge.1.0D0) x=1.0-1e-06
         !  nx1=nx_sp1+ksp1
         !  betap=cdbbsval(knotxr(1:nx1),ksp1,coefbeta(1:nx_sp1),x,jderivs)
          betap = inrcsval(xa_cs0,coefcsbeta(1:4,1:nx_cs0-1),x,jderivs)
          betap=b0_eff*betap
!          dum = inrcsval(gridx2,cscoefxpsi,x,0) ! psi
...
!           betapp=b0*( - 2.0*0.95- 3.0*4.0*0.05*x**2) ! ITPA n=6
          ...
          betapp=inrcsval(xa_cs0,coefcsbeta(1:4,1:nx_cs0-1),x,jderivs)
          betapp=b0_eff*betapp
        endfunction betapp
!
        function pnor_i(x) ! normalized pressure, n_i*T_i/(ni0*Ti0)
          implicit none
          real(r8) x, dum
          real(r8) :: pnor_i
          !dum = inrcsval(gridx2,cscoefxpsi,x,0) ! psi
...
!
       ...
       if(x.ge.1.0D0) x=1.0-1e-06
      !  nx1=nx_sp1+ksp1
      !  den=cdbbsval(knotxr(1:nx1),ksp1,coefden(1:nx_sp1),x,jderivs)
       den = inrcsval(xa_cs0,coefcsden,x,jderivs)
       !den=cden(1)
       !x1=x
...
!
       ...
       if(x.le.1.0D-3) x=1.0D-3
       if(x.ge.1.0D0) x=1.0-1e-06
       den_h_b = inrcsval(xa_cs0,coefcsnb,x,jderivs)
       end function den_h_b
!
       function t_h_b(x)
       implicit none
       integer,parameter :: jderivs=0
       ...
       if(x.le.1.0D-3) x=1.0D-3
       if(x.ge.1.0D0) x=1.0-1e-06
       t_h_b = inrcsval(xa_cs0,coefcstb,x,jderivs)
       end function t_h_b
!
       function den1(x)
       implicit none
...
!       den1=1.0D0
       ...
       if(x.ge.1.0D0) x=1.0-1e-06
      !  nx1=nx_sp1+ksp1
      !  denp=cdbbsval(knotxr(1:nx1),ksp1,coefden(1:nx_sp1),x,jderivs)
       denp= inrcsval(xa_cs0,coefcsden,x,jderivs)
       !xx=x**2
       !do i=1,nfit-1
...
!
       ...
       if(x.le.1.0D-3) x=1.0D-3
       if(x.ge.1.0D0) x=1.0-1e-06
       denp_h_b = inrcsval(xa_cs0,coefcsnb,x,jderivs)
       end function denp_h_b
!
      function tp_h_b(x)
       implicit none
       integer,parameter :: jderivs=1
       ...
       if(x.le.1.0D-3) x=1.0D-3
       if(x.ge.1.0D0) x=1.0-1e-06
       tp_h_b = inrcsval(xa_cs0,coefcsnb,x,jderivs)
       end function tp_h_b
!
!
       function tfun(x)
       ...
       integer,parameter :: jderivs=0
         real(r8) :: x,xx,tfun
       real(r8) :: f1,f2,te0,ti0
       if(x.le.1.0D-3) x=1.0D-3
       if(x.ge.1.0D0) x=1.0-1e-06
      ! nx1=nx_sp1+ksp1
      te0=1.734 ! temperature at axis kev
      ti0=2.098
      ! f1=cdbbsval(knotxr(1:nx1),ksp1,coefte(1:nx_sp1),x,jderivs)
      ! f2=cdbbsval(knotxr(1:nx1),ksp1,coefti(1:nx_sp1),x,jderivs)
       f1= inrcsval(xa_cs0,coefcste,x,jderivs)
       f2= inrcsval(xa_cs0,coefcsti,x,jderivs)
       tfun=te0/ti0*f1/f2
        !tfun=ctem(1)
        !xx=x**2
...
!  
      ...
      function tefun(x) ! Te
       ...
       if(x.ge.1.0D0) x=1.0-1e-06
      ! nx1=nx_sp1+ksp1
      ! tefun=cdbbsval(knotxr(1:nx1),ksp1,coefte(1:nx_sp1),x,jderivs)
         tefun= inrcsval(xa_cs0,coefcste(1:4,1:nx_cs0-1),x,jderivs)
       end function tefun
! 
!
       function tifun(x) ! Ti
       ...
       if(x.ge.1.0D0) x=1.0-1e-06
      !  nx1=nx_sp1+ksp1
      !  tifun=cdbbsval(knotxr(1:nx1),ksp1,coefti(1:nx_sp1),x,jderivs)
         tifun= inrcsval(xa_cs0,coefcsti(1:4,1:nx_cs0-1),x,jderivs)
        end function tifun
!
       ...
       implicit none
         real(r8) :: x
         real(r8) :: qfun,dq,cq2,dum,cq3,psi
         integer isp
         integer,parameter :: jderivs=0
         integer :: nx1
         if(x.le.1.0D-3) x=1.0D-3
         if(x.ge.1.0D0) x=1.0-1e-06
         !  nx1=nx_sp1+ksp1
         !  qfun=cdbbsval(knotxr(1:nx1),ksp1,coefqfun(1:nx_sp1),x,jderivs)
          qfun=inrcsval(xa_cs0,coefcsqfun(1:4,1:nx_cs0-1),x,jderivs)
         ...
         !dq=q1-q0
         !cq2=1.0D0-cq1
         !qfun=q0+dq*(cq1*x**2+cq2*x**4)  !GMEC Liu n_3
...
!  
       ...
       implicit none
         ...
         if(x.ge.1.0D0) x=1.0-1e-06
         ! nx1=nx_sp1+ksp1
         ! dqfun=cdbbsval(knotxr(1:nx1),ksp1,coefqfun(1:nx_sp1),x,jderivs)
         dqfun=inrcsval(xa_cs0,coefcsqfun(1:4,1:nx_cs0-1),x,jderivs)
...
!  
         ...
         !dq=q1-q0
         !cq2=1.0D0-cq1
         !dqfun=dq*(2.0D0*cq1*x+4.0D0*cq2*x**3)   !GMEC Liu n_3
...
!
      function ddqfun(x)
        ...
        integer,parameter :: jderivs=2
         ...
         if(x.ge.1.0D0) x=1.0-1e-06
      !  nx1=nx_sp1+ksp1
      !  ddqfun=cdbbsval(knotxr(1:nx1),ksp1,coefqfun(1:nx_sp1),x,jderivs)
        ddqfun=inrcsval(xa_cs0,coefcsqfun(1:4,1:nx_cs0-1),x,jderivs)
        !isp = 0 ! 0 for anlytic, 1 for interp
        !dq=-9.0D0
        !cq2=4.0D0
        !cq3=4.2D0
...
!  
        ...
        ! dq=q1-q0
        ! cq2=1.0D0-cq1
        ! ddqfun=dq*(2.0D0*cq1+12.0D0*cq2*x**2)  !GMEC Liu n_3
...
!
       ...
       END SUBROUTINE SIMP
!
 SUBROUTINE SIMP_COMPLEX(N, H, FI, S)
  IMPLICIT NONE
  INTEGER :: N, I
  REAL(r8) :: H
  COMPLEX(r8) :: S0, S1, S2, S
  COMPLEX(r8), DIMENSION(N) :: FI

  ! Initialize sums to zero (complex zero)
  S   = CMPLX(0.0_r8, 0.0_r8)
  S0  = CMPLX(0.0_r8, 0.0_r8)
  S1  = CMPLX(0.0_r8, 0.0_r8)
  S2  = CMPLX(0.0_r8, 0.0_r8)

  ! Simpson's rule summation with step 2
  DO I = 2, N-1, 2
     S1 = S1 + FI(I-1)
     S0 = S0 + FI(I)
     S2 = S2 + FI(I+1)
  END DO
  S = H * (S1 + 4.0_r8 * S0 + S2) / 3.0_r8

  ! If N is even, add the last slice separately
  IF (MOD(N, 2) .EQ. 0) THEN
     S = S + H * (5.0_r8*FI(N) + 8.0_r8*FI(N-1) - FI(N-2)) / 12.0_r8
  END IF

  RETURN
 END SUBROUTINE SIMP_COMPLEX
...
!          
         ...
         print *, 'prot0=',prot0,'prot=',prot,&
                  'zprt=',zprt,'zprt0=',zprt0
!
         ...
         !ne0 = 1.2e+13 ! HL-2A
         ti0 = 1.9995D0 !keV axis temperature of ions
         !ti0 = 2.0D0
...
!
        ...
        close(3100)
         ...
         omg = sqrt(lam_omg) ! normalized to vA_0/R0
         print *, 'omg=',omg
         nE1 = 101
         numL1 = 101
...
!
        ...
        subroutine profcoefdat
         ...
         parameter(k0=5)
!
          if (allocated(xa_cs0)) deallocate(xa_cs0)
          if (allocated(coefcste)) deallocate(coefcste)
          if (allocated(coefcsti)) deallocate(coefcsti)
          if (allocated(coefcsden)) deallocate(coefcsden)
          if (allocated(coefcsbeta)) deallocate(coefcsbeta)
          if (allocated(coefcsqfun)) deallocate(coefcsqfun)
          if (allocated(coefcsnb)) deallocate(coefcsnb)
          if (allocated(coefcsrot)) deallocate(coefcsrot)
          if (allocated(coefcstb)) deallocate(coefcstb)

         open(unit=530, file='fort.530', status='old')
         read(530,*) ! Reading the header or metadata from the file
         read(530,*)
         ! Read the spline degree (ksp) and the number of spline points (nx_sp) from the file
         read(530,*) nx_cs0
         allocate(xa_cs0(nx_cs0),coefcste(4,nx_cs0-1),coefcsti(4,nx_cs0-1))
         allocate(coefcsden(4,nx_cs0-1),coefcsbeta(4,nx_cs0-1))
         allocate(coefcsqfun(4,nx_cs0-1),coefcsnb(4,nx_cs0-1))
         allocate(coefcsrot(1:4,1:nx_cs0-1),coefcstb(1:4,1:nx_cs0-1))

         do i=1,nx_cs0
            !if (is_open(530)) then
                read(530,*) xa_cs0(i)
            !else
            !    print *, "Error: File 530 is not open."
            !    stop
            !endif
         enddo
         close(530)
!
         open(unit=540, file='fort.540', status='old')
         read(540,*) ! Reading coefficients for temperature (Te, Ti) and density (nb) profiles
         do i=1,nx_cs0-1
            read(540,'(12E16.8)') coefcste(1:4,i),coefcsti(1:4,i),coefcsnb(1:4,i)
         enddo
         close(540)
!
         open(unit=550, file='fort.550', status='old')
         read(550,*) ! Reading density profile coefficients from the file
         do i=1,nx_cs0-1
            read(550,'(4E16.8)') coefcsden(1:4,i)
         enddo
         close(550)
!
         open(unit=560, file='fort.560', status='old')
         read(560,*) ! Reading q-profile coefficients from the file
         do i=1,nx_cs0-1
            read(560,'(4E16.8)') coefcsqfun(1:4,i)
         enddo
         close(560)
!
         open(unit=570, file='fort.570', status='old')
         read(570,*) ! Reading p-profile coefficients from the file
         do i=1,nx_cs0-1
            read(570,'(4E16.8)') coefcsbeta(1:4,i)
         enddo
         close(570)
!
         open(unit=580, file='fort.580', status='old')
         read(580,*) ! Reading rotation profile beam temperature coefficients from the file
         do i=1,nx_cs0-1
            read(580,'(8E16.8)') coefcsrot(1:4,i), coefcstb(1:4,i)
         enddo
         close(580)
...
!
          if (allocated(xa_cs)) deallocate(xa_cs)
          if (allocated(coefcsc)) deallocate(coefcsc)
          if (allocated(coefcsaf0)) deallocate(coefcsaf0)
          if (allocated(coefcsbf0)) deallocate(coefcsbf0)
          if (allocated(coefcsaf)) deallocate(coefcsaf)
          if (allocated(coefcsbf)) deallocate(coefcsbf)
          if (allocated(coefcsgrr0)) deallocate(coefcsgrr0)
          if (allocated(coefcsgrr)) deallocate(coefcsgrr)
          if (allocated(coefcsgrt)) deallocate(coefcsgrt)
          if (allocated(coefcsgtt0)) deallocate(coefcsgtt0)
          if (allocated(coefcsgtt)) deallocate(coefcsgtt)
         ! allocate(xa_cs(nx0),coefcsc(4,nx0-1))
         ! allocate(coefcsaf(4,nx0-1,nf0),coefcsaf0(4,nx0-1))
         ! allocate(coefcsbf(4,nx0-1,nf0))
         ! allocate(coefcsgrr0(4,nx0-1),coefcsgrr(4,nx0-1,nc0))
         ! allocate(coefcsgtt0(4,nx0-1),coefcsgtt(4,nx0-1,nc0))
         ! allocate(coefcsgrt(4,nx0-1,nc0))


         open(unit=53, file='fort.53', status='old')
         read(53,*)
         read(53,'(3I8)') nx_cs,nc_cs,nf
         allocate(xa_cs(nx_cs),coefcsc(4,nx_cs-1))
         allocate(coefcsbf0(4,nx_cs-1),coefcsaf0(4,nx_cs-1))
         allocate(coefcsaf(4,nx_cs-1,nf),coefcsbf(4,nx_cs-1,nf))
         allocate(coefcsgrr0(4,nx_cs-1),coefcsgrr(4,nx_cs-1,nc_cs))
         allocate(coefcsgtt0(4,nx_cs-1),coefcsgtt(4,nx_cs-1,nc_cs))
         allocate(coefcsgrt(4,nx_cs-1,nc_cs))
         do i=1,nx_cs
            read(53,'(ES16.8)') xa_cs(i)
         enddo
         close(53)
!
         open(unit=54, file='fort.54', status='old')
           read(54,*) ! Skip header line '# coefgrr0 coefgtt0'
           do i=1,nx_cs-1
             read(54,'(8E16.8)') coefcsgrr0(1:4,i),coefcsgtt0(1:4,i)
           enddo
         close(54)
!
         open(unit=55, file='fort.55', status='old')
        read(55,*) 
        do i=1,nc_cs
           do j=1,nx_cs-1
             read(55,'(12E16.8)') coefcsgrt(1:4,j,i),coefcsgrr(1:4,j,i),coefcsgtt(1:4,j,i)
          enddo
        enddo
         close(55)

         open(unit=56, file='fort.56', status='old')
        read(56,*)
       do i=1,nx_cs-1
          read(56,'(4E16.8)') coefcsc(1:4,i)         
       enddo
         close(56)
!

         open(unit=57, file='fort.57', status='old')
!
       read(57,*)
       do i=1,nx_cs-1
          read(57,'(8E16.8)') coefcsaf0(1:4,i),coefcsbf0(1:4,i)
       enddo
         close(57)
!
         open(unit=58, file='fort.58', status='old')
        read(58,*)
        do j=1,nf
           do i=1,nx_cs-1
              read(58,'(8E16.8)') coefcsaf(1:4,i,j),coefcsbf(1:4,i,j)
           enddo
        enddo
        close(58)
!
       ...
       subroutine gyrosub(xrdum, lambda, dume, theta, zeta_j, delta_r_j, delta_theta_j)
            ! dume: energy of fast ions normalized ekev
            ! lambda: magnetic moment of fast ions
            ! xrdum: radial position of fast ions normalized radius
            ! theta: poloidal angle of fast ions
            ! zeta_j: gyro-angle of N points averaging
          implicit none
          real(8), intent(in) :: xrdum, lambda, dume, theta, zeta_j
          real(8), intent(out) :: delta_r_j, delta_theta_j
          real(8) :: d1, d2, dum3, delta_r, delta_theta, dum6
          real(8) :: fac

          fac = 1.0

         if (xrdum + fac * delta >= 1.0) then
             d1 = (rf(theta, xrdum - fac * delta) - rf(theta, xrdum))**2 + &
             (zf(theta, xrdum - fac * delta) - zf(theta, xrdum))**2
          else
             d1 = (rf(theta, xrdum + fac * delta) - rf(theta, xrdum))**2 + &
             (zf(theta, xrdum + fac * delta) - zf(theta, xrdum))**2
          endif
          d1 = sqrt(d1) ! normalized to R0
          d2 = (rf(theta + fac * dtheta, xrdum) - rf(theta, xrdum))**2 + &
          (zf(theta + fac * dtheta, xrdum) - zf(theta, xrdum))**2
          d2 = sqrt(d2) ! normalized to R0
          dum3 = sqrt(2.0 * lambda * dume / b_nor(theta, xrdum)) * rhoh_bar ! rho_c
          delta_r = dum3 / d1 * fac * delta
          delta_theta = dum3 / d2 * fac * dtheta
          dum6 = grt(theta, xrdum) / sqrt(grr(theta, xrdum) * gtt(theta, xrdum))
          dum6 = acos(dum6)
          delta_r_j = delta_r / sin(dum6) * sin(zeta_j + dum6 / 2.0)
          delta_theta_j = delta_theta / sin(dum6) * sin(zeta_j - dum6 / 2.0)
       end subroutine gyrosub
